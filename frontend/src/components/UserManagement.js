...
const UserManagement = () => {
    ...
    const [editingUserId, setEditingUserId] = useState(null); // 編輯模式的用戶 ID
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingUserId) {
                // 更新用戶
                await axios.put(`/users/${editingUserId}`, formData);
                alert('用戶更新成功');
            } else {
                // 新增用戶
            ...
            <div className="flex flex-col md:flex-row">
                ...
                <div className="md:w-1/3 md:mr-4">
                    <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow-md mb-6">
                        ...
                        <div className="mb-4">
                            <label className="block text-gray-700 mb-2" htmlFor="username">用戶名</label>
                            <input
                                type="text"
                                id="username"
                                name="username"
                        ...
                        <div className="mb-4">
                            <label className="block text-gray-700 mb-2" htmlFor="role">角色</label>
                            <select
                                id="role"
                                name="role"
                                value={formData.role}
                        ...
                        <div className="bg-white p-6 rounded shadow-md">
                            ...
                            <table className="min-w-full bg-white">
                                ...
                                <tbody>
                                    {users.length === 0 ? (
                                        <tr>
                                            <td colSpan="3" className="text-center py-4">沒有用戶資料</td>
                                        </tr>
                                    ) : (
                                        users.map((user) => (
                                            <tr key={user._id}>
